...
sudo sed -i 's|After=network.target|After=network.target home-ec2\\x2duser-airflow.mount|g' /usr/lib/systemd/system/airflow-webserver.service;
sudo sed -i 's|After=network.target|After=network.target home-ec2\\x2duser-airflow.mount|g' /usr/lib/systemd/system/airflow-scheduler.service;
sudo sed -i 's|After=network.target|After=network.target home-ec2\\x2duser-airflow.mount|g' /usr/lib/systemd/system/airflow-flower.service;

sudo mv /home/ec2-user/airflow /var/tmp/
sudo mkdir -p /home/ec2-user/airflow/
cd /home/ec2-user || exit
sudo mount -t nfs4 -o nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2 ${EFS_ENDPOINT_MASTER}:/ /home/ec2-user/airflow
sudo echo -e "${EFS_ENDPOINT_MASTER}:/ /home/ec2-user/airflow   nfs4 nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2 0 0" | sudo tee -a /etc/fstab
...
ami_config_ls=`ls /var/tmp/airflow/`
efs_config_ls=`ls /home/ec2-user/airflow/config`
echo "Content of /home/ec2-user/airflow/config after EFS mount: $${config_ls}"
